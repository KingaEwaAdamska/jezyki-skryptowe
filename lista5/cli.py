...
def valid_measurement(measurement_str):
    ...
    allowed = {
        ...
        'C6H6', 'Cd(PM10)', 'CO', 'DBahA(PM10)', 'Depozycja', 'formaldehyd', 'Hg(TGM)',
        'IP(PM10)', 'Jony', 'Ni(PM10)', 'NO2', 'NOx', 'NO', 'O3', 'Pb(PM10)',
        'PM10', 'PM25', 'PrekursoryZielonka', 'SO2'
    }
...
def parse_measurement_timestamp(timestamp):
    # Spotykane sa oba zapisy (MM/DD/YY i DD/MM/YY), wiec probujemy obu.
...
def get_station_measurements_in_range(file_path, station_code, start_date, end_date):
    ...
    station_data = next((item for item in data if item.station_code == station_code), None)
...
def main():
    parser = argparse.ArgumentParser(description='CLI do analizy zanieczyszczeń powietrza.')

    parser.add_argument('--measurement', required=True, type=valid_measurement, help='np. PM2.5, PM10, NO2')
    parser.add_argument('--mesurement', dest='measurement', type=valid_measurement, help=argparse.SUPPRESS)
    parser.add_argument('--frequency', required=True, choices=['1g', '24g', '1m'], help='Częstotliwość pomiarów: 1g (godzinowa), 24g (dobowa), 1m (miesięczna)')
    parser.add_argument('--start-date', required=True, type=valid_date, help='Data początkowa w formacie RRRR-MM-DD')
    parser.add_argument('--end-date', required=True, type=valid_date, help='Data końcowa w formacie RRRR-MM-DD')

    subparsers = parser.add_subparsers(dest='command', required=True, help='Dostępne polecenia')

    subparsers.add_parser('random', help='Wypisuje nazwę i adres losowej stacji dla danych parametrów')
    parser_stats = subparsers.add_parser('stats', help='Oblicza średnią i odchylenie standardowe')
    parser_stats.add_argument('--station', required=True, help="Kod stacji (np. DsWrocKorzA08)")
    ...
    if not target_file:
        print(f"Nie znaleziono pliku dla klucza: {key}. Sprawdz wielkosc, czestotliwosc i rok.")
        sys.exit(1)
    ...
    if args.command == 'random':
        active_codes = get_active_codes_from_file(target_file, args.start_date, end_date_inclusive)
